...
function sg_require_route_access(string $page): void
{
    ...
    $sellerApprovalRoutes = ['sell_ticket', 'active_listings', 'wallet', 'transaction'];
    if (in_array($page, $sellerApprovalRoutes, true) && !sg_user_has_approved_kyc((int) $_SESSION['user_id'])) {
        sg_flash('Pendaftaran seller kamu masih menunggu persetujuan admin. Lengkapi KTP dan selfie KTP dulu untuk membuka fitur jual.', 'error');
...
function sg_handle_kyc_submit(): void
{
    ...
    $returnPage = (string) ($_POST['return_page'] ?? 'settings');
    $successPage = (string) ($_POST['success_page'] ?? $returnPage);
    $allowedReturnPages = ['settings', 'seller_register'];
    $allowedSuccessPages = ['settings', 'seller_register', 'seller_overview'];
    if (!in_array($returnPage, $allowedReturnPages, true)) {
        $returnPage = 'settings';
    }
    if (!in_array($successPage, $allowedSuccessPages, true)) {
        $successPage = $returnPage;
    }
    ...
    if ($selfiePath === '') {
        sg_flash('Upload selfie dengan KTP wajib untuk pendaftaran seller.', 'error');
        sg_redirect($returnPage);
    }
    ...
    sg_flash($created ? 'Pendaftaran seller berhasil dikirim. Kamu bisa melihat dashboard seller, tetapi fitur jual aktif setelah admin menyetujui KYC.' : 'KYC gagal dikirim: ' . sg_db_error(), $created ? 'success' : 'error');
    sg_redirect($created ? $successPage : $returnPage);
}
