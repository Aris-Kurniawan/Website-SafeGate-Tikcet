...
function sg_get_user_kyc_submission(?int $userId): array
{
    if (!$userId || !sg_db()) {
        return [
            'id' => null,
    ...
    $submission = sg_fetch_one(
        'SELECT id, nik, ktp_photo_path, selfie_photo_path, status, submitted_at, reviewed_at, rejection_reason
...
function sg_user_has_approved_kyc(?int $userId): bool
{
